...
void display(element *stack,int top,int count)
{
    int i;
    if(top==-1)
    {
...
void pop_all_at_once(element *stack,int *count,int *top)
{
    int i;
    if(*top==-1)
    {
    ...
    printf("%d implies stack is empty\n",*top);
    printf("%d implies number of elements in the stack\n",*count);
}
...
void main()
{
    ...
    for(;;)
    {
        printf("1:push 2:pop 3:display 4:pop all at once 5:exit\n");
        ...
        case 4: pop_all_at_once(stack,&count,&top);
                break;
